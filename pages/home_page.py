import customtkinter as ctk

from backend.config import Config
from backend.start_setup import get_setup

_home_instance = None

def get_home():
    global _home_instance
    if _home_instance is None:
        _home_instance = Home()
    return _home_instance

class Home:
    def __init__(self):
        self.config = Config()
        self.config.main()
        self.setup = get_setup()

        #--- Widgets ---#
        self.level_label = None

        #--- Frames ---#
        self.frame = None
        self.focus_frame = None
        self.xpbar_frame = None
        self.coins_frame = None
        self.streak_frame = None
        self.gear_bonus_frame = None
        self.quests_completed_frame = None
        self.four_frames = None

        #--- Four Frames Widgets ---#
        self.coin_label = None
        self.coin_display = None
        self.coin_change_display = None
        self.topbar_coin_display = None
        self.topbar_habit_display = None
        self.streak_label = None
        self.gear_bonus_label = None
        self.quests_completed_label = None

    def main(self):
        self.frame = ctk.CTkFrame(self.setup.content_frame, width=1120, height=780, fg_color=self.config.bg, corner_radius=0)
        self.frame.grid(row=0, column=0, sticky="nsew")
        self.frame.grid_propagate(False)

        self.focus_frame = ctk.CTkFrame(self.frame, width=1050, height=200, fg_color=self.config.card, corner_radius=30)
        self.focus_frame.grid(row=0, column=0, padx=35, sticky="nsew")

        self.xpbar_frame = ctk.CTkFrame(self.frame, width=1050, height=50, fg_color=self.config.card, corner_radius=30)
        self.xpbar_frame.grid(row=1, column=0, padx=35, pady=(15,7), sticky="nsew")

        self.four_frames = ctk.CTkFrame(self.frame, width=1050, height=150, fg_color=self.config.bg, corner_radius=30)
        self.four_frames.grid(row=2, column=0, padx=35, sticky="nsew")

        self.four_frames.grid_propagate(False)

        self.coins_frame = ctk.CTkFrame(self.four_frames, width=240, height=130, fg_color=self.config.card, corner_radius=30)
        self.coins_frame.grid(row=0, column=0, pady=10, padx=(15, 10), sticky="nsew")
        self.coins_frame.grid_propagate(False)

        self.streak_frame = ctk.CTkFrame(self.four_frames, width=240, height=130, fg_color=self.config.card, corner_radius=30)
        self.streak_frame.grid(row=0, column=1, pady=10, padx=(10, 10), sticky="nsew")

        self.streak_frame.grid_propagate(False)

        self.gear_bonus_frame = ctk.CTkFrame(self.four_frames, width=240, height=130, fg_color=self.config.card, corner_radius=30)
        self.gear_bonus_frame.grid(row=0, column=2, pady=10, padx=(10, 10), sticky="nsew")

        self.gear_bonus_frame.grid_propagate(False)

        self.quests_completed_frame = ctk.CTkFrame(self.four_frames, width=240, height=130, fg_color=self.config.card, corner_radius=30)
        self.quests_completed_frame.grid(row=0, column=3, pady=10, padx=(10, 15), sticky="nsew")

        self.quests_completed_frame.grid_propagate(False)

